...
public class Physics {
	...
	private static PointF axis = new PointF(0, 0);
	...
	public static class CollisionInfo {
		static float depth;
		static PointF normal = new PointF(0, 0);
		static Edge e;
		static Vertex v;
	};
	...
	private void iterateCollisions() {
		for(int i = 0; i < iterations; i++ ) { //Repeat this a few times to give more exact results
			...
			for (int t = 0; t < vertexCount; t++ ) {
				PointF pos = vertices[ t ].position;
				pos.x = Math.max( Math.min( pos.x, screenWidth/2  ), -screenWidth/2 );
				pos.y = Math.max( Math.min( pos.y, screenHeight/2 ), -screenHeight/2 );
			}
			for (int j = 0; j < this.bodyCount; j++) {
				this.bodies[j].applyConstraints(); //Recalculate the center
			}
	...
	private static boolean detectCollision( PhysicsBody b1, PhysicsBody b2 ) {
		...
		if (b1.collisionDisabled || b2.collisionDisabled)
			return false;
	...
	public Physics(float width, float height, float GravitationX, float GravitationY, int pIterations) {
		this.screenWidth = width;
		this.screenHeight = height;
		this.bodyCount = 0;
		this.vertexCount = 0;
		this.edgeCount = 0;
		this.gravity = new PointF(GravitationX, GravitationY);
